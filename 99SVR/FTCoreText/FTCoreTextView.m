...
NSString *const FTCoreTextTagLink = @"a";
NSString *const FTCoreTextTagSpan = @"span";
...
- (void)processText
{
	...
	while (!finished) {
        ...
        tagName = [[tagName stringByTrimmingCharactersInSet:[NSCharacterSet characterSetWithCharactersInString:@"< />"]] lowercaseString];
        ...
        switch (tagType) {
            case FTCoreTextTagTypeOpen:
            {
                ...
                NSRange styleRange = [fullTag rangeOfString:@"(style=\"?(.*?)(\"|>|\\s+)|STYLE=\"?(.*?)(\"|>|\\s+))" options:NSRegularExpressionSearch];
                if (styleRange.location != NSNotFound) {
                    DLog(@"fullTag:%@",fullTag);
                    NSString *strStyle = [fullTag substringWithRange:styleRange];
                    DLog(@"strStyle:%@",strStyle);
                }
...
- (void)doInit
{
	...
	_defaultsTags = [NSMutableDictionary dictionaryWithObjectsAndKeys:FTCoreTextTagDefault, FTCoreTextTagDefault,
					 ...
					 FTCoreTextTagBullet, FTCoreTextTagBullet,FTCoreTextTagSpan,FTCoreTextTagSpan,nil];
    
    FTCoreTextStyle *defaultStyle = [[FTCoreTextStyle alloc] init];
    defaultStyle.name = FTCoreTextTagDefault;
    defaultStyle.textAlignment = FTCoreTextAlignementJustified;
    defaultStyle.font = XCFONT(15);
    
    FTCoreTextStyle *spanStyle = [[FTCoreTextStyle alloc] init];
    spanStyle.name = @"span";
    spanStyle.font = XCFONT(15);
    spanStyle.textAlignment = FTCoreTextAlignementLeft;
    
    FTCoreTextStyle *aStyle = [FTCoreTextStyle styleWithName:FTCoreTextTagLink];
    aStyle.name = @"a";
    aStyle.color = [UIColor blueColor];
    aStyle.font = XCFONT(15);
    ...
    [self addStyles:@[defaultStyle, imgStyle, spanStyle, aStyle]];
}
