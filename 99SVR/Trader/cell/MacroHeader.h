...
/**
 *  目前适配5，6，6p的尺寸。根据不同的机型，返回不同的尺寸。❗️❗️❗️这里一定要按照要求传进去。必须要传字符串。里面是数组下标就是5，6，6三个。后期如果是有其它屏幕的适配 在依次添加
 *  @"100,200,300" ===》iphone5 =100 iphone6=200 iPhone6 Plus = 300
 */
...
#define ValueWithTheIPhoneModelString(string) (G_IPhone__5 ? (IphoneStringWithIntValue(string,0)) : (G_IPhone__6 ? (IphoneStringWithIntValue(string,1)) :(IphoneStringWithIntValue(string,2))))
