...
-(void)chickEmptyViewShow:(NSMutableArray *)dataArray withCode:(NSString *)code{
    ...
    if (requestBool) {//请求成功
        ...
        if (dataArray.count==0) {
            ...
            if (dataArray == self.dayDataArray) {
                self.dayEmptyView = [ViewNullFactory createViewBg:self.dayTab.bounds imgView:[UIImage imageNamed:@"network_anomaly_fail@3x"] msg:@"日收益为请求失败"];
                [self.dayTab addSubview:self.dayEmptyView];
                
            }else if (dataArray == self.monDataArray){
                
                self.monEmptyView = [ViewNullFactory createViewBg:self.monTab.bounds imgView:[UIImage imageNamed:@"network_anomaly_fail@3x"] msg:@"月收益为请求失败"];
                [self.monTab addSubview:self.monEmptyView];
                
            }else{
                self.totalEmptyView = [ViewNullFactory createViewBg:self.totalTab.bounds imgView:[UIImage imageNamed:@"network_anomaly_fail@3x"] msg:@"总收益为请求失败"];
