...
{
public:
	virtual void onResponse(vector<QuestionAnswer>& info,int isteacher, int type);void OnError(int errCode) ;
};

class MailReplyListener : public HttpListener
{
public:
	virtual void onResponse(vector<MailReply>& info,int isteacher, int type);void OnError(int errCode) ;
};
...
{
public:
	virtual void onResponse2(std::vector<Team>& room_data, bool extra/*无用的参数,为多重继承而添加*/);void OnError(int errCode) ;
};
