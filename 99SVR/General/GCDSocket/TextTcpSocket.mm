...
- (void)reqViewMessage:(NSString *)strContent title:(NSString *)strTitle msgid:(int64)messageId
{
    ...
    ::strncpy(req->content,(const char*)titleData.bytes,titleData.length);
    ::strncpy(req->content+titleData.length,(const char*)contentData.bytes, contentData.length);
...
- (void)reqOperViewType:(int)viewTypeId name:(NSString *)strContent type:(int16_t)type
{
    ...
    ::strncpy(req.viewtypename,(const char*)typeData.bytes,nLength);
...
//        cBuffer[nSize-1] = '\0';
    ...
    //设置直播类型观点
    {
        ...
        ::strncpy(notify->srcuseralias,(const char*)teachdata.bytes,teachdata.length);
