...
@property (nonatomic,copy) NSString *strPlay;
@end
...
- (void)startPlayWithURLString:(NSString *)path
{
    ...
    ^{
        [__self.imageView makeToastActivity_2:@"bottom"];
    });
    _moviePosition = 0;
    if(_strPath==nil)
    {
    ...
    {
        if(![_strPath isEqualToString:path])
        {
            _strPlay = [NSString stringWithFormat:@"%@%@",_strPath,path];
        }
        else
        {
            _strPlay = _strPath;
    ...
    __weak NSString *__strPath = _strPlay;
    dispatch_async(dispatch_get_global_queue(0, 0),
    ^{
        NSError *error = nil;
        BOOL bFlag = [decoder openFile:__strPath error:&error];
        if (!bFlag)
        {
            dispatch_async(dispatch_get_main_queue(), ^{
                [weakSelf.imageView hideToastActivity];
                [weakSelf.imageView makeToast:@"连接失败"];
            });
        }
        else
        {
...
-(void) play
{
    ...
    dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
        [__self.imageView hideToastActivity];
        [__self tick];
    });
...
- (void) tick
{
    ...
    if (self.playing) {
        ...
        if (0 == leftFrames) {
            ...
            if (_minBufferedDuration > 0 && !_buffered && [_strPlay rangeOfString:@"only-audio"].location == NSNotFound)
            {
                _buffered = YES;
            }
        }
