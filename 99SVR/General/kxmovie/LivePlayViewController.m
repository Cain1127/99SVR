...
static LivePlayViewController *fsPlay;
...
//        {
//            [fsPlay stopWithError:VideoPlayErrorTypeHDError andError:nil];
//            return 0;
//        }
...
- (void)startPlayWithURLString:(NSString *)playURLString
{
    if(_strPath==nil)
    {
        _strPath = playURLString;
        input_filename = [playURLString UTF8String];
    }
    else
    {
        if(![_strPath isEqualToString:playURLString])
        {
            input_filename = [[NSString stringWithFormat:@"%@%@",_strPath,playURLString] UTF8String];
        }
        else
        {
            input_filename = [_strPath UTF8String];
        }
    }
    if (videoPlayState == VideoPlayStatePlaying || videoPlayState == VideoPlayStatePause)
    {
    ...
    if (!input_filename)
    {
        [fsPlay stopWithError:VideoPlayErrorTypeInput andError:nil];
    }
    ...
    if (SDL_Init (flags)) {
        //SDL错误
        [fsPlay stopWithError:VideoPlayErrorTypeSDLError andError:nil];
    }
    ...
    if (!is)
    {
        ...
        //do_exit(NULL);
        [fsPlay stopWithError:VideoPlayErrorTypeInitError andError:nil];
    }
    videoState = is;
...
- (void)showVideo
{
    if (isFrameOk)
    {
        ...
        {
            KxVideoFrameRGB *frame =nil;
            @synchronized(_aryVideo)
            {
                frame = [_aryVideo objectAtIndex:0];
...
- (void)createVideoFrame:(VideoState *)is
{
    ...
    sws_scale(_swsContext,
              (const uint8_t **)is->videoFrame->data,
              is->videoFrame->linesize,
              0,
...
- (void) frameView
{
    CGRect bounds = Rect(0, 1,kScreenWidth,kVideoImageHeight-1);
    if (!_glView)
    {
        _glView = [[UIImageView alloc] initWithFrame:bounds];
...
{
    [self startPlayWithURLString:strUrl];
}
