...
static NewPlayViewController *ffPlay;
...
static int queue_picture(VideoState *is, AVFrame *src_frame, double pts, double duration, int64_t pos, int serial)
{
    ...
    if (!vp->bmp || vp->reallocate || !vp->allocated ||
        ...
        //SDL_PeepEvents(&event, 1, SDL_GETEVENT, SDL_USEREVENT, SDL_LASTEVENT)
...
void connectFileInfo(const char *input_file)
{
    int flags;
    VideoState *is;
    ...
    is = stream_open(input_file, file_iformat);
    if (!is)
    {
...
{
    connectFileInfo([playURLString UTF8String]);
}
