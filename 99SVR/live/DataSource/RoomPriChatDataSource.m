...
@interface RoomPriChatDataSource()<DTAttributedTextContentViewDelegate>
{
    NSCache *chatCache;
}
...
@implementation RoomPriChatDataSource
...
- (void)linkPushed:(DTLinkButton *)sender
{
    ...
    if([sender.URL.absoluteString rangeOfString:@"sqchatid://"].location != NSNotFound)
    {
        ...
        if (_delegate && [_delegate respondsToSelector:@selector(priChatShowKeyboard:)]) {
            [_delegate priChatShowKeyboard:toUser];
        }
    }
}
